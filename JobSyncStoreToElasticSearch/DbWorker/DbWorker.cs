        ...
        public static void Fill(DataSet dataSet, string procedureName, int rp = -1)
        {
            using (SqlConnection oConnection = new SqlConnection(_connection))
            {
                ...
                using (SqlTransaction oTransaction = oConnection.BeginTransaction())
                {
                    ...
                    {
                        //String er = "<========|| storedName: " + procedureName;
                        //LogsWriter.WriteLog(HttpContext.Current.Server.MapPath("~"), "Error", er);
                        //LogsWriter.WriteLog(HttpContext.Current.Server.MapPath("~"), "Error" + ex.ToString());
                        ErrorWriter.WriteLog(startupPath, "Procedure Fill Data Table with param " + ex.ToString());
                        oTransaction.Rollback();
                        throw;
                    }
                ...
                if (oConnection.State != ConnectionState.Closed)
                    // LogsWriter.WriteLog(HttpContext.Current.Server.MapPath("~"), "Procedure Fill DataSET ", oConnection.State.ToString());
                    ErrorWriter.WriteLog(startupPath, "Procedure Fill Data Table with param", oConnection.State.ToString());
            }
        }
